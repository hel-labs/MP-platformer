...
### Added
- **Coin system + sprite** (`Coin.java`, `CoinManager.java`, `ObjectManager.java`, `res/coin_sprites.png`)
    - Coins spawn, render, and are tracked by the object manager.

### Fixed
- **Game Over restart resets to level 1** (`Playing.java`, `GameOverOverlay.java`)
    - Added `restartCurrentLevel()` and wired Game Over play button to use it.

### Changed
- **State/util package layout** (`gamestate/*`, `utils/*`)
    - Overworld state/UI/effects/util classes consolidated under `gamestate` and `utils` packages.
- **Level cap** (`LevelManager.java`)
    - Limit levels to `MAX_LEVELS = 3` when loading from `/res/lvls`.
