...
/**
 * Screen shake effect.
 * Call trigger() when something impactful happens.
 * Call update(dt) each tick.
 * Apply getOffsetX/Y to the Graphics2D transform before rendering.
 */
public class ScreenShake {
    ...
    public int getOffsetX() { return offset(); }
    public int getOffsetY() { return offset(); }

    private int offset() {
        if (timer >= duration || intensity == 0) return 0;
        float strength = intensity * (1f - timer / duration);
        return (int)(rand.nextGaussian() * strength);
    }

    public boolean isActive() { return timer < duration; }
}
